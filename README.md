...
管理台是服务拥有者的全权限控制台，提供工作台、分类管理、凭据管理、时效复核与回收站五个视图：可浏览、检索、新建、编辑、软删除、延期复核并恢复全库所有分类下的记忆。分类管理支持新建分类、就地编辑名称描述与安全删除空分类；分类标识（slug）创建后不可修改，以保证记忆外键始终有效。凭据管理支持查看 Key 状态、权限矩阵、在线签发（单次明文披露）、吊销与物理删除。新增或编辑分类后，工作台的筛选胶囊与「新建记忆」抽屉的分组选项即时更新。
