...
| 三态授权：授权、不可见、不存在；未授权分组不披露存在性 | 独立用户表、多角色 RBAC 与 Cookie/Session |
| 时效复核、回收站软删除与恢复 | 分组标识（slug）创建后的修改 |
| 全权限单管理员 Web 管理台：工作台、分类管理、凭据管理、时效复核、回收站 | 回收站条目的物理清理 |
| Web 端分类创建、就地编辑与安全删除（仅空分类可删） | 备份的异地对象存储同步 |
| Web 端凭据深度管理：查看 Key 状态、权限矩阵、在线签发（单次明文披露）、吊销与物理删除 | 环境变量管理员凭据的 Web 化修改（改值重启） |
| 在线热备与快照回灌 | |
...
管理台采用 Microsoft Fluent 2 设计系统打造（侧栏导航 + 标题栏 App Shell、Mica 画布底色、Acrylic 半透明框架层与深浅色运行时切换），是服务拥有者的全权限控制台，提供工作台、分类管理、凭据管理、时效复核与回收站五个视图：可浏览、检索、新建、编辑、软删除、延期复核并恢复全库所有分类下的记忆。分类管理支持新建分类、就地编辑名称描述与安全删除空分类；分类标识（slug）创建后不可修改，以保证记忆外键始终有效。凭据管理支持查看 Key 状态、权限矩阵、在线签发（单次明文披露）、吊销与物理删除。新增或编辑分类后，工作台的筛选胶囊与「新建记忆」抽屉的分组选项即时更新。
